...
const ImageUploader = forwardRef<HTMLImageElement, ImageUploaderProps>(({ id, label, onFileSelect, imageUrl }, ref) => {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (!allowedTypes.includes(file.type)) {
        setFileTypeError('For best results, please use PNG or JPG formats.');
      } else {
        setFileTypeError(null);
      }
      onFileSelect(file);
  ...
  const uploaderClasses = `w-full aspect-video bg-zinc-100 border-2 border-dashed rounded-lg flex items-center justify-center transition-all duration-300 relative overflow-hidden ${
    ...
    : 'border-zinc-300'
  } ${!imageUrl ? 'hover:border-blue-500 cursor-pointer' : 'cursor-default'}`;
